...
function tagTree(inv, tree, chip) {
  ...
  return Object.values(groups).map(g => ({ ...g, isTag: true, notTag: false, askAndi: () => {}, hasMark: false, noMark: false, gpu: false, open: true, toggle: () => {}, caret: 'rotate(90deg)', sub: `${Object.keys(g.regions).length} regions across ${new Set(Object.keys(g.regions).map(k => k.split(' ')[0])).size} clouds · ${g.wl.toLocaleString('en-US')} workloads · one policy covers all of it`, badge: { label: g.priv ? 'all on the fabric' : 'some on the public internet', bg: g.priv ? '#eef8f0' : 'var(--bg-wash)', border: g.priv ? '#8fd4a4' : 'var(--border-secondary)', color: g.priv ? '#1e7a3c' : 'var(--text-body)' }, regions: Object.values(g.regions) })).sort((a, b) => b.wl - a.wl);
}
