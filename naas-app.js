...
function addendumVals(c, s, set, est, ob, inv, go, findingCard, totalSave, est0) {
  ...
  const sk = ob.sankey;
  const skNodes = sk.nodes.map((n, i) => { const left = n.side !== 'r'; const door = n.kind === 'site' ? go('s1', { siteOpen: { [n.cls]: true }, treeOrMap: 'tree' }) : n.kind === 'tag' ? go('s1', { tagView: true, treeOrMap: 'tree' }) : n.kind === 'region' ? () => set({ andiScope: { kind: 'region', id: n.name.split(' ').slice(1).join(' '), label: n.name }, andiOpen: true }) : n.kind === 'dest' ? () => set({ obTab: 'control' }) : null; return { ...n, key: 'k' + i, fill: n.side === 'm' ? (n.priv ? '#0057b8' : (dark ? '#5d6f80' : '#8a949c')) : (dark ? '#c5cfd9' : '#1a2431'), lx: left ? n.x2 + 6 : n.x - 226, ly: n.y + n.h / 2 - 10, lw: 220, justify: left ? 'flex-start' : 'flex-end', label: n.name, vF: n.v.toFixed(1) + ' Gbps', go: door || (() => {}), cursor: door ? 'pointer' : 'default', title: door ? (n.kind === 'site' ? 'Open in Explore 360' : n.kind === 'tag' ? 'Open By tag in Explore 360' : n.kind === 'region' ? 'Ask Andi about this region' : 'Open the flow logs') : '' }; });
