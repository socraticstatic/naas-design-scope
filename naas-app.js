...
export function vals(c) {
  ...
  const heroSites = L.sites.map(st => ({ ...st, key: st.key, op: dimFor(['site' + st.name]), ty: st.y + 15, ty2: st.y + 29, dash: st.ghost || st.more ? '4 4' : 'none', color: st.ghost ? 'var(--text-disabled)' : st.more ? 'var(--link)' : 'var(--text-heading)', click: () => { if (st.ghost || st.leaf) return; if (st.more) { go('s1')(); return; } const key = st.drillKey || (S.countOf(st.name) > 1 || st.rollup ? S.classOf(st) : st.name); set({ drill: [...s.drill, key] }); }, enter: () => set({ hoverNode: 'site' + st.name }), leave: () => set({ hoverNode: null }), cursor: st.ghost || st.leaf ? 'default' : 'pointer' }));
  ...
  const crumbs = [{ key: 'floor', label: 'Floor', go: go('s2', { drill: [], cloudDrill: [] }) }, ...s.drill.map((d, i) => ({ key: 'd' + i, label: crumbLabel(d), go: () => set({ drill: s.drill.slice(0, i + 1) }) })), ...cloudDrill.map((d, i) => ({ key: 'c' + i, label: i === 0 ? d : (regionDrill && i === cloudDrill.length - 1 ? regionDrill.label.split(' › ').pop() : d), go: () => set({ cloudDrill: cloudDrill.slice(0, i + 1) }) }))];
...
function shellVals(s, set, go, est, c) {
  ...
  const railGroups = top === 'ai'
    ...
    : [
        ...
        { key: 'observe', hasTitle: true, title: 'Observe', items: [
          ...
          item('Security & Governance', 'check-shield', () => { go('s3', { layer: 'cloud', tab: 'observe' })(); set({ obPage: 'sec', obTab: 'flow' }); }, onS3('cloud', 'observe') && s.obPage === 'sec' && obTabNow !== logsTab),
        ] },
        { key: 'deep', hasTitle: true, title: 'Deep dive', items: [
          item('Logs', 'checklist', () => { go('s3', { layer: 'cloud', tab: 'observe' })(); set({ obPage: 'perf', obTab: logsTab }); }, onS3('cloud', 'observe') && obTabNow === logsTab),
        ] },
        { key: 'govern', hasTitle: true, title: 'Govern', items: [
  ...
  const pageTitle = s.screen === 's1' ? 'Explore 360' : s.screen === 's4' ? 'Compose' : s.screen === 's5' ? 'Recommend' : s.screen === 's6' ? 'Review order' : storeCur ? 'Marketplace'
    : s.screen === 's3' ? (s.layer === 'ai' ? ({ connect: 'AI Fabric', govern: 'Policies', observe: aiTitle, cost: 'Budget & Limits' }[s.tab] || 'AI Fabric') : ({ connect: 'Fabric', govern: 'Policies', observe: (obTabNow === logsTab ? 'Logs' : s.obPage === 'sec' ? 'Security & Governance' : 'Performance & Reliability'), cost: 'Cost' }[s.tab] || 'NaaS'))
    : 'NaaS';
