...
export function vals(c) {
  ...
  const volList = vol ? V.volumeList(est0, vol, volOpts) : null;
  ...
  const drawer = volList ? { ...volList, key: 'vol', q: s.volQ || '', setQ: (e) => set({ volQ: e.target.value, volPage: 1 }), close: closeVolume,
    paths: [['all', 'All'], ['fabric', 'On the fabric'], ['public', 'Public']].map(([k, l]) => ({ key: k, label: l, on: (s.volPath || 'all') === k, go: () => set({ volPath: k, volPage: 1 }), bg: (s.volPath || 'all') === k ? 'var(--cta)' : 'var(--bg-base)', color: (s.volPath || 'all') === k ? '#fff' : 'var(--text-heading)', border: (s.volPath || 'all') === k ? 'var(--cta)' : 'var(--border-secondary)' })),
    states: [['all', 'Any state'], ['degraded', 'Degraded']].map(([k, l]) => ({ key: k, label: l, on: (s.volState || 'all') === k, go: () => set({ volState: k, volPage: 1 }), bg: (s.volState || 'all') === k ? 'var(--cta)' : 'var(--bg-base)', color: (s.volState || 'all') === k ? '#fff' : 'var(--text-heading)', border: (s.volState || 'all') === k ? 'var(--cta)' : 'var(--border-secondary)' })),
    rows: volList.rows.map(x => ({ ...x, key: x.id, dot: x.state === 'degraded' ? 'var(--error)' : x.state === 'public' ? 'var(--warning)' : 'var(--success)', pinned: s.volPin === x.id, bg: s.volPin === x.id ? 'var(--bg-accent)' : 'transparent', toggle: () => set({ volSel: (s.volSel || []).includes(x.id) ? (s.volSel || []).filter(k => k !== x.id) : [...(s.volSel || []), x.id] }), pin: () => set({ volPin: x.id, mapSel: 'asset:' + x.id, panelTab: 'overview', drill: s.drill.length >= 2 ? s.drill : [vol.cls, vol.metro] }), hasAction: !!x.action, act: x.action === 'Attach' ? () => { c.setState({ screen: 's4', compose: { ...prefillCompose(est), bulk: `${x.id} · ${x.address}` }, parsedNote: `Attach ${x.id} (${x.address}) in ${x.metro}: one circuit onto the fabric.` }); syncHash('s4', s.layer, s.tab); } : () => set({ andiScope: { kind: 'region', id: x.id, label: x.id }, andiOpen: true }) })),
    ...
    selectAll: () => set({ volSel: volList.matchingIds }), clearSel: () => set({ volSel: [] }), hasSel: (s.volSel || []).length > 0, bulkLabel: `Attach ${volList.bulk.attach.toLocaleString('en-US')} · ${volList.bulk.label}`, canBulk: volList.bulk.attach > 0, bulkAttach, matchingF: volList.matching.toLocaleString('en-US') } : null;
  ...
  const heroDefault = s.screen === 's0' || s.screen === 's2' || (s.screen === 's3' && s.layer === 'cloud' && s.tab !== 'observe');
  ...
  const crumbs = [{ key: 'floor', label: 'Home', go: go('s2', { drill: [], cloudDrill: [] }) }, ...s.drill.map((d, i) => ({ key: 'd' + i, label: crumbLabel(d), go: () => set({ drill: s.drill.slice(0, i + 1) }) })), ...((regionDrill && regionDrill.crumb) || cloudDrill).map((name, i) => ({ key: 'c' + i, label: name, go: () => set({ cloudDrill: cloudDrill.slice(0, Math.max(1, i)) }) }))];
