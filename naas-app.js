...
export function vals(c) {
  ...
  const drawer = volList ? { ...volList, key: 'vol', q: s.volQ || '', setQ: (e) => set({ volQ: e.target.value, volPage: 1 }), close: closeVolume,
    ...
    rows: volList.rows.map(x => ({ ...x, key: x.id, dot: x.state === 'degraded' ? 'var(--error)' : x.state === 'public' ? 'var(--warning)' : 'var(--success)', pinned: s.volPin === x.id, bg: s.volPin === x.id ? 'var(--bg-accent)' : 'transparent', toggle: () => set({ volSel: (s.volSel || []).includes(x.id) ? (s.volSel || []).filter(k => k !== x.id) : [...(s.volSel || []), x.id] }), pin: () => set({ volPin: s.volPin === x.id ? null : x.id, drill: s.drill.length >= 2 ? s.drill : [vol.cls, vol.metro] }), hasAction: !!x.action, act: x.action === 'Attach' ? () => { c.setState({ screen: 's4', compose: { ...prefillCompose(est), bulk: `${x.id} · ${x.address}` }, parsedNote: `Attach ${x.id} (${x.address}) in ${x.metro}: one circuit onto the fabric.` }); syncHash('s4', s.layer, s.tab); } : () => set({ andiScope: { kind: 'region', id: x.id, label: x.id }, andiOpen: true }) })),
