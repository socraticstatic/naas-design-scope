...
export function vals(c) {
  ...
  const openWorkloads = (region, vpcId, snId) => set({ vol: { kind: 'workloads', region, vpcId, snId: snId || null }, drawerOpen: true, andiOpen: false, volQ: '', volPath: 'all', volState: 'all', volPage: 1, volSel: [] });
  ...
  const drawer = volList ? { ...volList, key: 'vol', searchHint: volList.kind === 'workloads' ? 'Search name, ip, type, app' : 'Search id, street, host', q: s.volQ || '', setQ: (e) => set({ volQ: e.target.value, volPage: 1 }), close: closeVolume,
