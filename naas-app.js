...
export function vals(c) {
  ...
  const fabRows = fabInfo ? fabInfo.rows.slice(0, 9).map((r, i) => ({ ...r, key: r.key, y: L.bandY + 34 + i * 30, dot: FAB_STATE[r.state] || FAB_STATE.ok, caret: r.leaf ? '' : '›', cursor: r.leaf ? 'default' : 'pointer', click: () => { if (r.leaf) { set({ andiScope: { kind: 'region', id: fabDrill[1], label: r.name }, andiOpen: true }); return; } set({ fabDrill: [...fabDrill, r.drill] }); } })) : [];
  const fabHead = fabInfo ? { label: fabInfo.label, head: fabInfo.head, more: fabInfo.rows.length > 9 ? `+${fabInfo.rows.length - 9} more` : '' } : null;
  ...
  return {
    ...
    // hero
    fabOpen: fabDrill.length > 0, fabClosed: fabDrill.length === 0, fabRows, fabHead, fabUp, fabTrail, hasFabMore: !!(fabHead && fabHead.more), fabMore: fabHead ? fabHead.more : '', fabHeadY: L.bandY + 8,
...
function addendumVals(c, s, set, est, ob, inv, go, findingCard, totalSave, est0) {
  ...
  const patterns = [['all', 'All'], ...F.PATTERNS].map(([k, l]) => ({ key: k, label: l, on: mapPattern === k, go: () => set({ mapPattern: k }), bg: mapPattern === k ? 'var(--cta)' : 'var(--bg-base)', color: mapPattern === k ? '#fff' : 'var(--text-heading)', border: mapPattern === k ? 'var(--cta)' : 'var(--border-secondary)' }));
